...
function Get-AssemblyFileVersion([string]$semanticVersion) {
  if ([string]::IsNullOrWhiteSpace($semanticVersion)) {
    return "0.0.0.0"
  }

  $numberMatches = [regex]::Matches($semanticVersion, '\d+')
  $parts = @()
...
$fileVersion = Get-AssemblyFileVersion -semanticVersion $Version
& dotnet publish $serviceProject -c Release -r win-x86 --self-contained true -o $stageRuntimeRoot "/p:Version=$Version" "/p:InformationalVersion=$Version" "/p:FileVersion=$fileVersion" "/p:AssemblyVersion=$fileVersion"
