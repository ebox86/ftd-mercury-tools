...
function Get-ServiceSecurityDescriptor([string]$name) {
  $output = & sc.exe sdshow $name 2>&1
  if ($LASTEXITCODE -ne 0) {
    throw ("Unable to read service security descriptor for '{0}' (exit={1})." -f $name, $LASTEXITCODE)
  }

  $text = (($output | ForEach-Object { "$_" }) -join [Environment]::NewLine).Trim()
  $match = [Regex]::Match($text, 'D:.*')
  if (-not $match.Success) {
    throw ("Service '{0}' returned an unexpected security descriptor response: {1}" -f $name, $text)
  }

  return $match.Value.Trim()
}

function Test-ServiceAceContainsRights {
  param(
    [Parameter(Mandatory = $true)]
    [string]$Rights,
    [Parameter(Mandatory = $true)]
  ...
  foreach ($code in $RequiredCodes) {
    if ($Rights.IndexOf($code, [StringComparison]::OrdinalIgnoreCase) -lt 0) {
      return $false
    }
  }
...
function Has-InteractiveServiceControlAce([string]$sddl) {
  if ([string]::IsNullOrWhiteSpace($sddl)) {
    return $false
  }

  $matches = [Regex]::Matches($sddl, '\(A;;([^;]*);;;IU\)')
  foreach ($match in $matches) {
    ...
    # Require start (RP), stop (WP), and query-status (LC).
    if (Test-ServiceAceContainsRights -Rights $rights -RequiredCodes @('RP', 'WP', 'LC')) {
      return $true
    }
  }
...
function Grant-InteractiveServiceControl([string]$name) {
  # Grant interactive users the minimum rights needed for tray lockstep:
  # query-status (LC), start (RP), stop (WP), interrogate (LO).
  $requiredAce = '(A;;RPWPLOLC;;;IU)'
  $currentSddl = Get-ServiceSecurityDescriptor -name $name

  if (Has-InteractiveServiceControlAce -sddl $currentSddl) {
    Write-Host "Service '$name' already grants interactive start/stop rights."
    return
  }

  $saclIndex = $currentSddl.IndexOf('S:', [StringComparison]::Ordinal)
  $updatedSddl = if ($saclIndex -ge 0) {
    $currentSddl.Substring(0, $saclIndex) + $requiredAce + $currentSddl.Substring($saclIndex)
  }
  else {
    $currentSddl + $requiredAce
  }

  Invoke-Sc -ScArgs @('sdset', $name, $updatedSddl)

  $verifySddl = Get-ServiceSecurityDescriptor -name $name
  if (-not (Has-InteractiveServiceControlAce -sddl $verifySddl)) {
    throw "Failed to verify interactive start/stop rights on service '$name' after sdset."
  }

  Write-Host "Granted interactive start/stop rights for service '$name'."
}
...
function Install-OrUpdateService {
  ...
  $binPath = ('"{0}" --port={1} --logical-name={2} --scanner-mode={3} --claim-timeout-ms={4} --log-level={5} --enable-scanner --enable-printer' -f $resolvedExe, $Port, $LogicalName, $ScannerMode, $ClaimTimeoutMs, $LogLevel)
  $service = Get-ServiceWmi -name $ServiceName
  ...
  Grant-InteractiveServiceControl -name $ServiceName
