...
{
  public string Version { get; init; } = DetectRuntimeVersion();
  ...
  public bool EnableScanner { get; init; }
  public bool EnablePrinter { get; init; }
  ...
  public static BridgeOptions FromArgs(string[] args)
  {
    ...
    {
      Version = ResolveString(kv, "version", "FTD_OPOS_VERSION", DetectRuntimeVersion()),
      ...
      EnableScanner = flags.Contains("enable-scanner"),
      EnablePrinter = flags.Contains("enable-printer"),
  ...
  private static string DetectRuntimeVersion()
  {
    try
    {
      ...
      {
        return TrimBuildMetadata(informational);
      }
