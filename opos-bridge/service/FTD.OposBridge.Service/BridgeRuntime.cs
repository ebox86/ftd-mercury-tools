...
public sealed class BridgeRuntime : BackgroundService
{
  ...
  public async Task<PrinterOperationResult> PrintReceiptAsync(string data, CancellationToken cancellationToken)
  {
    if (_printerDriver == null)
    {
      return new PrinterOperationResult(
        Ok: false,
    ...
    var success = await _printerDriver.PrintAsync(data, prefs, cancellationToken);
    return new PrinterOperationResult(
      Ok: success,
      Error: success ? "" : (_printerDriver.LastError ?? "Print failed."),
      LogicalName: prefs.PrinterLogicalName ?? "",
      Message: "");
  }

  public async Task<PrinterOperationResult> PrintTestReceiptAsync(CancellationToken cancellationToken)
  {
    return await PrintTestReceiptAsync("", "", cancellationToken);
  }

  public async Task<PrinterOperationResult> PrintTestReceiptAsync(string logicalNameOverride, string messageOverride, CancellationToken cancellationToken)
  {
    if (_printerDriver == null)
    {
      return new PrinterOperationResult(
    ...
    var prefs = PrinterPreferences.Load();
    if (!string.IsNullOrWhiteSpace(logicalNameOverride))
    {
      prefs.PrinterLogicalName = logicalNameOverride.Trim();
    }
    ...
    var success = await _printerDriver.PrintAsync(message, prefs, cancellationToken);
    return new PrinterOperationResult(
      Ok: success,
      Error: success ? "" : (_printerDriver.LastError ?? "Test print failed."),
      LogicalName: prefs.PrinterLogicalName ?? "",
      Message: message);
  }
  ...
  {
    var scanner = _driver is null
      ? new ScannerStartupDiagnostics(
        Mode: _options.EnableScanner ? "unavailable" : "disabled",
        ...
        LastError: _options.EnableScanner ? "Scanner driver is unavailable." : "Scanner support disabled (--enable-scanner was not set).")
      : await _driver.GetStartupDiagnosticsAsync(cancellationToken);
    ...
    try
    {
      if (_driver is null)
      {
        return new
        {
          ...
          scannerStatus = _options.EnableScanner ? "error" : "disabled",
          lastSeq = _lastSeq,
        };
      }
  ...
  protected override async Task ExecuteAsync(CancellationToken stoppingToken)
  {
    ...
    try
    {
      if (_driver is null)
      {
        _scannerStatus = _options.EnableScanner ? "error" : "disabled";
        _scannerClaimed = false;
        _scannerDeviceEnabled = null;
        _scannerAutoDisable = null;
        _lastDriverErrorSeen = "";
        _lastError = _options.EnableScanner
          ? "Scanner driver is unavailable."
          : "Scanner support disabled (--enable-scanner was not set).";
        _logger.LogWarning(
          "Bridge started without scanner driver (enableScanner={EnableScanner}, logicalName={LogicalName}, port={Port}, pid={Pid}).",
          _options.EnableScanner,
          _options.LogicalName,
          _options.Port,
          _processId);
        _observability.Warn(_lastError, 2105);
      }
      else
      {
  ...
  private async Task<bool> ReleaseClaimInternalAsync(string reason, CancellationToken cancellationToken)
  {
    if (_driver is null)
    {
      ...
      _scannerAutoDisable = null;
      ClearLeaseTrackingInternal();
      UpdateAgentCommandInternal(false, $"release-no-driver:{reason}");
      return false;
    }
  ...
  public sealed record PrinterOperationResult(
    bool Ok,
    string Error,
    string LogicalName,
    string Message);
