...
if (-not $NoTrayCompanion.IsPresent -and -not $usedTaskFallback) {
  if ($TrayOnLogin) {
    ...
    Write-Host "TrayOnLogin is false: ensuring tray companion scheduled task is removed ..."
    try {
      & $taskInstallerPath -Action uninstall -TaskName $TrayTaskName | Out-Null
      Write-Host "Tray companion scheduled task removed."
    } catch {
